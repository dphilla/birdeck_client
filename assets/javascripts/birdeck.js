...
// in refactoring: import 'ajax-request.js'
...
$(document).ready(function(){
  ...
  var getSinglePost = function() {
    var postId = $(".show_form input[name='show-id']").val();
    return $.ajax({
   ...
   var createNewPost = function( ){
    var postDesciption =  $(".post-form input[name='post-description']").val();
    return $.ajax({
      url: API + '/api/v1/posts',
      method: 'POST',
      data: { post: {description: postDesciption}}
    }).done(function(data) {
      console.log(data);
      for(i = 0; data.length; i++) {
        $('#latest-posts').append('<p class="post">' + data[i].description + '</p>');
  ...
  var updatePost = function(){
    var postDesciption =  $(".update-form input[name='post-description']").val();
